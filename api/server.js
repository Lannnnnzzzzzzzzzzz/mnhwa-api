...
// DATA GENRE
  app.get('/api/genres', async (req, res) => {
    try {
        const url = 'https://komikstation.org/manga/list-mode/'; // Replace with the actual URL
        const { data } = await axios.get(url);
        const $ = load(data);
...
app.get('/api/manhwa-ongoing', async (req, res) => {
  try {
      const url = 'https://komikstation.org/manga/?status=ongoing&type=manhwa&order=';
...
app.get('/api/chapter/:chapterId', async (req, res) => {
  ...
  const url = `https://komikstation.org/${chapterId}`; // Adjust the URL if necessary
